...
- **⚡ Real-Time Streaming Output (SSE)**: Streams translations word-by-word via Server-Sent Events for zero-latency feedback.
...
- **🔊 Text-to-Speech (TTS)**: Built-in high-quality voice playback supporting Persian (`fa-IR`), English (`en-US`), and other major languages.
...
- **🤖 Live Dynamic Model Fetching**: Query Google's `/v1beta/models` REST endpoint live from the Options page to dynamically load and select available Gemini models (`gemini-flash-latest`, `gemini-3.6-flash`, `gemini-3.5-flash`, `gemini-2.0-flash`).
...
├── content.js            # Injected web content script (floating tooltip, SSE streaming, drawer)
...
- **کشوی تاریخچه ترجمه‌ها درون خود صفحه وب و اکستنشن**
